...
### 第七轮：精炼补完 + 发布物（用户指出 §12.17 自评过早）

> 第六轮我把「精炼」自评为 ✅。用户指出**并未完成**。独立只读审计（逐字节
> 跨文件重复检测 + 引用计数式死代码检测）证实用户是对的。

| 维度 | 第六轮自评 | 第七轮实测 | 处置 |
|---|---|---|---|
| 精炼 | ~~✅~~ **过早** | 仍存 5 类跨文件重复 | 净 **−1339 行**，删 13 个文件 |
| 文档正确性 | 未验证 | README 示例 **4 处不编译**；OS 矩阵 **2 处写错** | 新增 **9 条文档路径门禁测试**，当场抓出并修正 |
...
|---|---|---|
| `#[cfg]` 属性自身重复 | 47 处 | 折叠 |
| `/proc`、`/sys` 探针逐字节重复 | **296 行，5 个后端** | 新建 `platform::os_probes` |
| 日期工具（`MONTH_NAMES` 是第三份名字清单）| 3 个文件 | 新建 `misc_widgets::date_utils` |
| `floor_char_boundary` | 6 份 | 新建 `widget::text_utils` |
| `Default` 样板 | **117 处，90 个文件** | 新增 `impl_default_via_new!` |
| `NativeCapabilityContract` ≡ `PlatformCapabilities` | 2 类型 + 1 拷贝函数 | 改为 `pub type` 别名 |
| 死代码模块（零引用）| **1261 行** | 删除 |
| 死 re-export 层 + 空文件 + 过期宏 | 5 + 5 + 1 | 删除 |
...
删除前已对 7 个候选符号逐一 `grep` 复核（`src/`/`tests/`/`examples/` 均 0 引用），
保留意见已记入日志 §13.3。
