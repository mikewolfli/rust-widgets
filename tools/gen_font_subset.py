...
def subset_bytes(source, face, license_id):
    ...
    instancer.instantiateVariableFont(font, face["instance"], inplace=True, updateFontNames=False)
    ...
    subsetter.populate(unicodes=face["unicodes"])
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--face", action="append", default=None,
                        help=f"which face to generate (repeatable); default all of {sorted(FACES)}")
    ...
    face_ids = args.face or sorted(FACES)
    for face_id in face_ids:
        if face_id not in FACES:
            print(f"unknown --face={face_id!r}; known: {', '.join(sorted(FACES))}", file=sys.stderr)
            return 1
    ...
    for face_id in face_ids:
        ...
        payload = subset_bytes(source, face, args.license_id)
