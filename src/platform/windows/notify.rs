...
pub(crate) fn ensure_window_class_registered() {
    ...
    REGISTERED.get_or_init(|| unsafe {
        ...
        wnd_class.lpfnWndProc = Some(super::types::wnd_proc);
...
///
/// The mapping itself lives in [`crate::platform::windows_notify`] so that it is
/// type-checked and unit-tested on every host, not only on Windows.
#[cfg(target_os = "windows")]
pub(crate) fn control_notify_kind_for_widget(
    kind: WindowsHandleKind,
    notify_code: u32,
) -> Option<WidgetTriggerKind> {
    crate::platform::windows_notify::control_notify_kind_for_widget(kind, notify_code)
}
...
pub(crate) fn enqueue_control_notify_event(
    ...
    if let Ok(mut events) = platform.menu_state.pending_widget_events.lock() {
        ...
        if crate::platform::windows_notify::combo_selchange_is_dual_emitted(widget_kind, kind) {
            events
...
///
/// Delegates to the host-compilable `windows_notify` module.
#[cfg(target_os = "windows")]
pub(crate) fn notify_kind_for_widget(
    kind: WindowsHandleKind,
    notify_code: u32,
) -> Option<WidgetTriggerKind> {
    crate::platform::windows_notify::notify_kind_for_widget(kind, notify_code)
}
