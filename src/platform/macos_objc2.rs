...
impl Platform for MacOSObjc2Platform {
    fn backend_name(&self) -> &'static str { "macos-objc2-preview" }
    ...
    fn create_menu(&self, parent: u64, text: &str, x: i32, y: i32, width: u32, height: u32) -> u64 {
        if !matches!(self.kind_of(parent), Some(MacObjc2HandleKind::MenuBar | MacObjc2HandleKind::Menu)) {
            return 0;
        }
...
mod tests {
    ...
    #[test]
    fn objc2_preview_backend_state_roundtrip() {
        ...
        assert!(button > 0);
        assert_eq!(backend.get_widget_text(button), "ok");

        backend.set_widget_text(button, "updated");
        assert_eq!(backend.get_widget_text(button), "updated");

        assert!(backend.set_clipboard_text("clip"));
        assert_eq!(backend.get_clipboard_text(), "clip");
    }
}
