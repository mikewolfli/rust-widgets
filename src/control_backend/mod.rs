...
//!
//! An earlier header claimed this file was generated by
//! `tools/split_control_backend.py` and told readers not to edit it. **That script
//! does not exist**, and neither does the `implementation.rs` it supposedly read
//! (the one-off migration that produced this layout was removed after it ran — see
//! `docs/log/log-20260908-1.md` and `docs/split1.md`). The instruction was therefore
//! unactionable: regenerating was impossible, and editing was the only way the file
//! could change. Edit it directly.
//!
//! # Reachability
//!
//! **State:** Production callers: `src/bindings/binding_impl.rs:1` (every `rw_create_*` funnels through `get_control_backend`).
