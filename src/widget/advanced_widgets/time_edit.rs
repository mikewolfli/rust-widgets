...
impl Time {
    ...
    pub fn to_msecs_since_midnight(&self) -> u32 {
        (self.hour as u32 * 3600 + self.minute as u32 * 60 + self.second as u32) * 1000 + self.msec as u32
    }
}
...
impl TimeEdit {
    ...
    pub fn minimum_time(&self) -> Time { self.minimum }
    pub fn maximum_time(&self) -> Time { self.maximum }
    ...
    pub fn set_minimum_time(&mut self, time: Time) { self.minimum = time; }
    pub fn set_maximum_time(&mut self, time: Time) { self.maximum = time; }
    pub fn set_time_range(&mut self, min: Time, max: Time) { self.minimum = min; self.maximum = max; }
    ...
    pub fn step_up(&mut self) {
        ...
        if t.minute >= 60 { t.minute = 0; if t.hour < 23 { t.hour += 1; } }
        self.set_time(t);
    }
    ...
    pub fn step_down(&mut self) {
        ...
        if t.second > 0 { t.second -= 1; }
            ...
            t.second = 59;
            if t.minute > 0 { t.minute -= 1; }
            else { t.minute = 59; if t.hour > 0 { t.hour -= 1; } }
        }
        self.set_time(t);
    }
}
