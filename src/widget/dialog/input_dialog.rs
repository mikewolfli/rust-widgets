...
impl InputDialog {
    ...
    pub fn get_text(geometry: Rect, title: impl Into<String>, label: impl Into<String>, default: impl Into<String>) -> Self {
    ...
    pub fn get_int(geometry: Rect, title: impl Into<String>, label: impl Into<String>, value: i64, min: i64, max: i64, step: i64) -> Self {
        let mut d = Self::new(geometry);
        d.title = title.into();
        d.label_text = label.into();
    ...
    pub fn text_value(&self) -> &str { &self.text_value }
    pub fn int_value(&self) -> i64 { self.int_value }
    pub fn double_value(&self) -> f64 { self.double_value }
    pub fn current_item(&self) -> usize { self.current_item }
    pub fn current_item_text(&self) -> Option<&str> { self.items.get(self.current_item).map(|s| s.as_str()) }
    ...
    pub fn set_text_value(&mut self, v: impl Into<String>) { self.text_value = v.into(); }
    pub fn set_items(&mut self, items: Vec<String>) { self.items = items; self.current_item = 0; }
    pub fn set_int_value(&mut self, v: i64) { self.int_value = v.clamp(self.int_min, self.int_max); }
    pub fn set_double_value(&mut self, v: f64) { self.double_value = v.clamp(self.double_min, self.double_max); }
...
impl Draw for InputDialog {
    fn draw(&self, context: &mut RenderContext) {
        ...
        context.fill_rect(rect.x + 10.0, input_y, rect.width - 20.0, 26.0, Color::from_rgb(255, 255, 255));
        context.draw_rect(rect.x + 10.0, input_y, rect.width - 20.0, 26.0, Color::from_rgb(150, 150, 150));

        let display_text = match self.mode {
            InputMode::Text => self.text_value.clone(),
            InputMode::Integer => self.int_value.to_string(),
            InputMode::Double => format!("{:.prec$}", self.double_value, prec = self.double_decimals as usize),
            InputMode::Item => self.current_item_text().unwrap_or("").to_string(),
        };
        context.draw_text(rect.x + 14.0, input_y + 13.0, &display_text, &Font::default(), Color::from_rgb(0, 0, 0), Alignment::Left);
