...
impl ProgressDialog {
    ...
    pub fn label_text(&self) -> &str { &self.label_text }
    pub fn value(&self) -> i32 { self.value }
    pub fn minimum(&self) -> i32 { self.minimum }
    pub fn maximum(&self) -> i32 { self.maximum }
    pub fn was_canceled(&self) -> bool { self.was_canceled }
    pub fn auto_close(&self) -> bool { self.auto_close }
    pub fn auto_reset(&self) -> bool { self.auto_reset }
    pub fn cancel_button_text(&self) -> &str { &self.cancel_button_text }

    pub fn set_title(&mut self, t: impl Into<String>) { self.title = t.into(); }
    pub fn set_label_text(&mut self, t: impl Into<String>) { self.label_text = t.into(); }
    pub fn set_minimum(&mut self, min: i32) { self.minimum = min; }
    pub fn set_maximum(&mut self, max: i32) { self.maximum = max; }
    pub fn set_range(&mut self, min: i32, max: i32) { self.minimum = min; self.maximum = max; }
    pub fn set_auto_close(&mut self, v: bool) { self.auto_close = v; }
    pub fn set_auto_reset(&mut self, v: bool) { self.auto_reset = v; }
    pub fn set_cancel_button_text(&mut self, t: impl Into<String>) { self.cancel_button_text = t.into(); }
...
impl Draw for ProgressDialog {
    fn draw(&self, context: &mut RenderContext) {
        ...
        context.draw_text(rect.x + 8.0, rect.y + 14.0, &self.title, &Font::default(), Color::from_rgb(255, 255, 255), Alignment::Left);

        // Label
        context.draw_text(rect.x + 10.0, rect.y + 48.0, &self.label_text, &Font::default(), Color::from_rgb(0, 0, 0), Alignment::Left);

        // Progress bar
        let bar_y = rect.y + 62.0;
        let bar_w = rect.width - 20.0;
        let bar_h = 20.0;
        context.fill_rect(rect.x + 10.0, bar_y, bar_w, bar_h, Color::from_rgb(220, 220, 220));
        context.draw_rect(rect.x + 10.0, bar_y, bar_w, bar_h, Color::from_rgb(150, 150, 150));
        let fill_w = bar_w * self.progress_fraction();
        if fill_w > 0.0 {
            context.fill_rect(rect.x + 10.0, bar_y, fill_w, bar_h, Color::from_rgb(6, 176, 37));
        }

        // Percentage text
        let pct = (self.progress_fraction() * 100.0) as i32;
        context.draw_text(
            rect.x + 10.0 + bar_w / 2.0, bar_y + bar_h / 2.0,
            &format!("{}%", pct), &Font::default(), Color::from_rgb(0, 0, 0), Alignment::Center,
        );

        // Cancel button
        let btn_y = rect.y + rect.height - 40.0;
        let btn_w = 80.0;
        context.fill_rect(rect.x + rect.width / 2.0 - btn_w / 2.0, btn_y, btn_w, 28.0, Color::from_rgb(225, 225, 225));
        context.draw_rect(rect.x + rect.width / 2.0 - btn_w / 2.0, btn_y, btn_w, 28.0, Color::from_rgb(100, 100, 100));
        context.draw_text(rect.x + rect.width / 2.0, btn_y + 14.0, &self.cancel_button_text, &Font::default(), Color::from_rgb(0, 0, 0), Alignment::Center);
    }
}
