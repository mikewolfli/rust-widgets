...
impl FileDialog {
    ...
    pub fn mode(&self) -> FileDialogMode { self.mode }
    pub fn title(&self) -> &str { &self.title }
    pub fn directory(&self) -> &str { &self.directory }
    pub fn selected_files(&self) -> &[String] { &self.selected_files }
    pub fn selected_file(&self) -> Option<&str> { self.selected_files.first().map(|s| s.as_str()) }
    pub fn name_filters(&self) -> &[FileFilter] { &self.name_filters }
    pub fn current_filter(&self) -> Option<&FileFilter> { self.name_filters.get(self.current_filter) }
    ...
    pub fn set_directory(&mut self, dir: impl Into<String>) { self.directory = dir.into(); }
...
impl Draw for FileDialog {
    fn draw(&self, context: &mut RenderContext) {
        ...
        context.fill_rect(rect.x + 10.0, list_y, rect.width - 20.0, list_h, Color::from_rgb(255, 255, 255));
        context.draw_rect(rect.x + 10.0, list_y, rect.width - 20.0, list_h, Color::from_rgb(150, 150, 150));
        context.draw_text(rect.x + 16.0, list_y + 20.0, "(file list)", &Font::default(), Color::from_rgb(150, 150, 150), Alignment::Left);

        // Selected files display
        let sel_y = list_y + list_h + 8.0;
        context.draw_text(rect.x + 10.0, sel_y + 10.0, "File name:", &Font::default(), Color::from_rgb(0, 0, 0), Alignment::Left);
        let fname = self.selected_file().unwrap_or("");
        context.fill_rect(rect.x + 80.0, sel_y, rect.width - 90.0, 22.0, Color::from_rgb(255, 255, 255));
        context.draw_rect(rect.x + 80.0, sel_y, rect.width - 90.0, 22.0, Color::from_rgb(150, 150, 150));
        context.draw_text(rect.x + 84.0, sel_y + 11.0, fname, &Font::default(), Color::from_rgb(0, 0, 0), Alignment::Left);

        // OK/Cancel buttons
        let btn_y = rect.y + rect.height - 40.0;
        let btn_w = 80.0;
        let ok_label = if self.mode == FileDialogMode::SaveFile { "Save" } else { "Open" };
        context.fill_rect(rect.x + rect.width - 176.0, btn_y, btn_w, 28.0, Color::from_rgb(0, 120, 215));
        context.draw_text(rect.x + rect.width - 136.0, btn_y + 14.0, ok_label, &Font::default(), Color::from_rgb(255, 255, 255), Alignment::Center);
        context.fill_rect(rect.x + rect.width - 88.0, btn_y, btn_w, 28.0, Color::from_rgb(225, 225, 225));
        context.draw_rect(rect.x + rect.width - 88.0, btn_y, btn_w, 28.0, Color::from_rgb(100, 100, 100));
