...
pub enum BubbleTailDirection { TopLeft, TopRight, BottomLeft, BottomRight, Left, Right, Top, Bottom }

#[derive(Debug, Clone, Copy, PartialEq)]
pub enum PathSegment { MoveTo(Point), LineTo(Point), CurveTo(Point,Point,Point), QuadTo(Point,Point), Close }
...
impl FreeformShapeWidget {
    ...
    pub fn path(&self) -> &ShapePath { &self.path }
    pub fn set_path(&mut self, path: ShapePath) { self.path = path; self.base.request_redraw(); }
    pub fn fill_color(&self) -> Color { self.fill_color }
    pub fn set_fill_color(&mut self, color: Color) { self.fill_color = color; self.base.request_redraw(); }
    pub fn stroke_color(&self) -> Option<Color> { self.stroke_color }
    pub fn set_stroke_color(&mut self, color: Option<Color>) { self.stroke_color = color; self.base.request_redraw(); }
    pub fn stroke_width(&self) -> u32 { self.stroke_width }
    pub fn set_stroke_width(&mut self, width: u32) { self.stroke_width = width; self.base.request_redraw(); }

    pub fn contains(&self, point: Point) -> bool {
        let rect = self.base.geometry();
        if !rect.contains_point(point) { return false; }
        ...
        match &self.path {
            ...
            ShapePath::Star { points, inner_radius } => self.contains_star(local, w, h, *points, *inner_radius),
            ...
            ShapePath::Bubble { tail_direction } => self.contains_bubble(local, w, h, *tail_direction),
    ...
    fn contains_star(&self, local: Point, w: i32, h: i32, num_points: u8, inner_ratio: f32) -> bool {
        let n = num_points.max(3) as usize;
        let cx = w as f32 / 2.0; let cy = h as f32 / 2.0;
        let outer_r = (cx.min(cy as f32)).max(1.0);
        ...
        for i in 0..n {
            ...
            vertices.push(Point::new((cx + outer_r * angle.cos()).round() as i32, (cy + outer_r * angle.sin()).round() as i32));
            let ia = angle + step;
            vertices.push(Point::new((cx + inner_r * ia.cos()).round() as i32, (cy + inner_r * ia.sin()).round() as i32));
        }
        self.ray_cast(local, &vertices)
    }

    fn contains_polygon(&self, local: Point, vertices: &[Point]) -> bool { self.ray_cast(local, vertices) }

    fn contains_rounded_rect(&self, local: Point, w: i32, h: i32, radius: u32) -> bool {
        let r = (radius as i32).min(w / 2).min(h / 2);
        if r <= 0 { return local.x >= 0 && local.y >= 0 && local.x < w && local.y < h; }
        let (x, y) = (local.x, local.y);
        if x < 0 || y < 0 || x >= w || y >= h { return false; }
        if x < r && y < r { return (x - r).pow(2) + (y - r).pow(2) <= r * r; }
        if x >= w - r && y < r { return (x - (w - r)).pow(2) + (y - r).pow(2) <= r * r; }
        if x < r && y >= h - r { return (x - r).pow(2) + (y - (h - r)).pow(2) <= r * r; }
        if x >= w - r && y >= h - r { return (x - (w - r)).pow(2) + (y - (h - r)).pow(2) <= r * r; }
        true
    }

    fn contains_bubble(&self, local: Point, w: i32, h: i32, td: BubbleTailDirection) -> bool {
        let ts = 12i32; let (x, y) = (local.x, local.y);
        ...
        if x < x_min || y < y_min || x >= x_max || y >= y_max { return false; }
        let r = 8u32.min(w as u32 / 2).min(h as u32 / 2).max(1);
        if x >= 0 && x < w && y >= 0 && y < h { return self.contains_rounded_rect(local, w, h, r); }
    ...
    fn contains_custom(&self, local: Point, segments: &[PathSegment]) -> bool {
        ...
        let mut pv = Vec::new(); let mut cur = Point::new(0, 0); let mut start = Point::new(0, 0);
        for seg in segments {
            match seg {
                PathSegment::MoveTo(p) => { if pv.len() > 1 { pv.push(start); } start = *p; cur = *p; pv.push(*p); }
                PathSegment::LineTo(p) => { cur = *p; pv.push(*p); }
                PathSegment::CurveTo(c1, c2, p) => { self.flatten_cubic_to(&mut pv, cur, *c1, *c2, *p); cur = *p; }
                PathSegment::QuadTo(c, p) => { self.flatten_quad_to(&mut pv, cur, *c, *p); cur = *p; }
                PathSegment::Close => { if pv.last() != Some(&start) { pv.push(start); } cur = start; }
            }
        }
        if pv.len() > 1 && pv.last() != Some(&start) { pv.push(start); }
        if pv.len() < 3 { return false; }
        self.ray_cast(local, &pv)
    }

    fn point_in_triangle(&self, pt: Point, a: Point, b: Point, c: Point) -> bool {
        let s = |px: f32, py: f32, x1: f32, y1: f32, x2: f32, y2: f32| -> f32 { (px - x2) * (y1 - y2) - (x1 - x2) * (py - y2) };
        let (ax, ay, bx, by, cx, cy, px, py) = (a.x as f32, a.y as f32, b.x as f32, b.y as f32, c.x as f32, c.y as f32, pt.x as f32, pt.y as f32);
    ...
    fn flatten_cubic_to(&self, vertices: &mut Vec<Point>, p0: Point, p1: Point, p2: Point, p3: Point) {
        let flatness_sq = 4.0;
        let dx = p3.x - p0.x; let dy = p3.y - p0.y;
        let d2 = ((p1.x - p3.x) * dy - (p1.y - p3.y) * dx).abs() as f32;
        let d3 = ((p2.x - p3.x) * dy - (p2.y - p3.y) * dx).abs() as f32;
        if (d2 + d3).powi(2) <= flatness_sq * (dx * dx + dy * dy) as f32 + 0.0001 { vertices.push(p3); return; }
    ...
    fn flatten_quad_to(&self, vertices: &mut Vec<Point>, p0: Point, p1: Point, p2: Point) {
        ...
        if d * d <= flatness_sq * (dx * dx + dy * dy) as f32 + 0.0001 { vertices.push(p2); return; }
        let lerp = |a: i32, b: i32| -> i32 { ((a as f32 + b as f32) / 2.0).round() as i32 };
        let mid1 = Point::new(lerp(p0.x, p1.x), lerp(p0.y, p1.y));
        let mid2 = Point::new(lerp(p1.x, p2.x), lerp(p1.y, p2.y));
    ...
    fn draw_shape(&self, context: &mut RenderContext) {
        ...
        match &self.path {
            ...
            ShapePath::Star { points, inner_radius } => self.draw_star(context, rect, *points, *inner_radius),
            ...
            ShapePath::Bubble { tail_direction } => self.draw_bubble(context, rect, *tail_direction),
    ...
    fn draw_heart(&self, context: &mut RenderContext, rect: Rect) {
        ...
        for x in blx..=brx {
            ...
            if y0 <= tip_y { context.fill_rect(Rect::new(x, y0, 1, (tip_y - y0 + 1) as u32), self.fill_color); }
        }
    ...
    fn draw_star(&self, context: &mut RenderContext, rect: Rect, num_points: u8, inner_ratio: f32) {
        ...
        for i in 0..n {
            let angle = i as f32 * 2.0 * std::f32::consts::PI / n as f32 - std::f32::consts::FRAC_PI_2;
            vertices.push(Point::new((cx as f32 + outer_r * angle.cos()).round() as i32, (cy as f32 + outer_r * angle.sin()).round() as i32));
            let ia = angle + step;
            vertices.push(Point::new((cx as f32 + inner_r * ia.cos()).round() as i32, (cy as f32 + inner_r * ia.sin()).round() as i32));
        }
        for i in 0..vertices.len() { let j = (i + 1) % vertices.len(); self.fill_triangle_approx(context, Point::new(cx, cy), vertices[i], vertices[j]); }
        if let Some(sc) = self.stroke_color { for i in 0..vertices.len() { let j = (i + 1) % vertices.len(); context.draw_line_stroke(vertices[i], vertices[j], sc, self.stroke_width); } }
    }

    fn draw_polygon(&self, context: &mut RenderContext, rect: Rect, vertices: &[Point]) {
        if vertices.len() < 3 { return; }
        let centroid = Point::new(vertices.iter().map(|v| v.x).sum::<i32>() / vertices.len() as i32, vertices.iter().map(|v| v.y).sum::<i32>() / vertices.len() as i32);
        let vs: Vec<Point> = vertices.iter().map(|v| Point::new(rect.x + v.x, rect.y + v.y)).collect();
        for i in 0..vs.len() { let j = (i + 1) % vs.len(); self.fill_triangle_approx(context, centroid, vs[i], vs[j]); }
        if let Some(sc) = self.stroke_color { for i in 0..vs.len() { let j = (i + 1) % vs.len(); context.draw_line_stroke(vs[i], vs[j], sc, self.stroke_width); } }
    }

    fn draw_rounded_rect(&self, context: &mut RenderContext, rect: Rect, radius: u32) {
        let r = radius.min(rect.width / 2).min(rect.height / 2);
        if r > 0 { context.fill_rounded_rect(rect, r, self.fill_color); } else { context.fill_rect(rect, self.fill_color); }
        if let Some(sc) = self.stroke_color { if r > 0 { context.draw_rounded_rect_stroke(rect, r, sc, self.stroke_width); } else { context.draw_rect_stroke(rect, sc, self.stroke_width); } }
    }
    ...
    fn draw_bubble(&self, context: &mut RenderContext, rect: Rect, td: BubbleTailDirection) {
        ...
        let (tcx, tcy, tdx, tdy) = match td {
            ...
            BubbleTailDirection::Right => (rect.x + rect.width as i32 - 1, rect.y + rect.height as i32 / 2, 1, 0),
            BubbleTailDirection::Top => (rect.x + rect.width as i32 / 2, rect.y, 0, -1),
            BubbleTailDirection::Bottom => (rect.x + rect.width as i32 / 2, rect.y + rect.height as i32 - 1, 0, 1),
            ...
            BubbleTailDirection::BottomRight => (rect.x + rect.width as i32 - 1, rect.y + rect.height as i32 - 1, 1, 1),
        };
        let tip = Point::new(tcx + tdx * ts, tcy + tdy * ts);
        let (bdx, bdy) = match td {
            BubbleTailDirection::Left | BubbleTailDirection::Right => (0, ts / 2),
        ...
        let b1 = Point::new(tcx + bdx, tcy + bdy); let b2 = Point::new(tcx - bdx, tcy - bdy);
    ...
    fn draw_custom(&self, context: &mut RenderContext, rect: Rect, segments: &[PathSegment]) {
        ...
        let centroid = Point::new(vs.iter().map(|v| v.x).sum::<i32>() / vs.len() as i32, vs.iter().map(|v| v.y).sum::<i32>() / vs.len() as i32);
        for i in 0..vs.len() { let j = (i + 1) % vs.len(); self.fill_triangle_approx(context, centroid, vs[i], vs[j]); }
        if let Some(sc) = self.stroke_color { for i in 0..vs.len() { let j = (i + 1) % vs.len(); context.draw_line_stroke(vs[i], vs[j], sc, self.stroke_width); } }
    }
    ...
    fn fill_triangle_approx(&self, context: &mut RenderContext, v0: Point, v1: Point, v2: Point) {
        ...
        if a.y == c.y {
            ...
            if max_x > min_x { context.fill_rect(Rect::new(min_x, a.y, (max_x - min_x) as u32, 1), self.fill_color); }
            return;
        }
        let total_height = c.y - a.y;
        if total_height <= 0 { return; }
        let lerp_x = |p1: Point, p2: Point, t: f32| -> f32 { p1.x as f32 + (p2.x - p1.x) as f32 * t };
        for y in a.y..=c.y {
            let (x1, x2) = if y < b.y {
                let sub_h = b.y - a.y;
                if sub_h == 0 { continue; }
                (lerp_x(a, c, (y - a.y) as f32 / total_height as f32), lerp_x(a, b, (y - a.y) as f32 / sub_h as f32))
            } else {
                let sub_h = c.y - b.y;
                if sub_h == 0 { continue; }
                (lerp_x(a, c, (y - a.y) as f32 / total_height as f32), lerp_x(b, c, (y - b.y) as f32 / sub_h as f32))
            };
            let x_start = x1.min(x2).round() as i32;
            let x_end = x1.max(x2).round() as i32;
            if x_end > x_start { context.fill_rect(Rect::new(x_start, y, (x_end - x_start) as u32, 1), self.fill_color); }
        }
    }
    ...
    fn ray_cast(&self, point: Point, vertices: &[Point]) -> bool {
        ...
        for i in 0..n {
            let vi = vertices[i];
            let vj = vertices[j];
            if ((vi.y > point.y) != (vj.y > point.y)) && (point.x as i64) < ((vj.x as i64 - vi.x as i64) * (point.y as i64 - vi.y as i64) / (vj.y as i64 - vi.y as i64) + vi.x as i64) {
                inside = !inside;
            }
            j = i;
        }
