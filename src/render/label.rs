...
impl LabelRenderer {
    ...
    fn draw_high_quality(context: &mut RenderContext, rect: Rect, text: &str, alignment: Alignment) {
        if text.is_empty() {
            return;
        }
        ...
        let text_color = context.current_text_color().unwrap_or(Color::from_rgb(0, 0, 0));
        ...
        if alignment == Alignment::Center || alignment == Alignment::Right {
            let shadow_rect = Rect::new(text_rect.x + 1, text_rect.y + 1, text_rect.width, text_rect.height);
            context.draw_shaped_text(shadow_rect, &shaped_text, Color::from_rgba(0, 0, 0, 30));
        }
    }
    
    /// Medium quality rendering with basic text.
    fn draw_medium_quality(context: &mut RenderContext, rect: Rect, text: &str, alignment: Alignment) {
        if text.is_empty() {
            return;
        }
    ...
    fn draw_low_quality(context: &mut RenderContext, rect: Rect, text: &str, alignment: Alignment) {
        ...
        let text_rect = Rect::new(rect.x + 2, rect.y + (rect.height as i32 / 2) - 6, rect.width, rect.height);
    ...
    fn calculate_text_rect(rect: Rect, shaped_text: &crate::render::ShapedText, alignment: Alignment) -> Rect {
        let text_width = shaped_text.advance() as i32;
        let text_height = 12; // Standard text height
        
        match alignment {
            Alignment::Left => Rect::new(rect.x + 2, rect.y + (rect.height as i32 - text_height) / 2, rect.width, rect.height),
            Alignment::Center => {
                let x = rect.x + (rect.width as i32 - text_width) / 2;
                Rect::new(x, rect.y + (rect.height as i32 - text_height) / 2, rect.width, rect.height)
            }
            Alignment::Right => {
                let x = rect.x + rect.width as i32 - text_width - 2;
                Rect::new(x, rect.y + (rect.height as i32 - text_height) / 2, rect.width, rect.height)
    ...
    fn batch_draw_group(context: &mut RenderContext, labels: &[(Rect, &str)], alignment: Alignment) {
