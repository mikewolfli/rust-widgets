//! TabWidget Demo - Tab container widget demonstration with event logging.
...
fn main() {
    ...
    match runtime_gui_mode() {
        RuntimeGuiMode::NativeInteractive => {
            eprintln!("[demo_tabwidget] running in native-interactive mode");
        }
        RuntimeGuiMode::PreviewOrStub => {
            eprintln!("[demo_tabwidget] preview/stub mode");
        }
    }

    let window = create_window(&tr("tabwidget.title"), 100, 100, 900, 700);
    ...
    let add_btn = create_button(window, &tr("tabwidget.add_tab"), 10, y, 120, btn_height);
    log_event("Button", add_btn, "Created (Add Tab)");

    let remove_btn = create_button(window, &tr("tabwidget.remove_tab"), 140, y, 120, btn_height);
    log_event("Button", remove_btn, "Created (Remove Tab)");

    y += 40;

    let tab_widget_panel = create_panel(window, 10, y, 880, 480);
    log_event("Panel", tab_widget_panel, "Created (Tab Widget Area)");

    y += 490;

    let log_edit = create_line_edit(window, "", 10, y, 880, 100);
    log_event("TextEdit", log_edit, "Created (Event Log)");

    let status_bar = create_status_bar(window, &tr("status.ready"), 0, 676, 900, 24);
    log_event("StatusBar", status_bar, "Created");

    show_widget(window);

    log_event("Window", window, "Shown - TabWidget Demo Started");
