//! DockPanel Demo - Dock panel container widget demonstration with event logging.
...
fn main() {
    ...
    match runtime_gui_mode() {
        RuntimeGuiMode::NativeInteractive => {
            eprintln!("[demo_dockpanel] running in native-interactive mode");
        }
        RuntimeGuiMode::PreviewOrStub => {
            eprintln!("[demo_dockpanel] preview/stub mode");
        }
    }

    let window = create_window(&tr("dockpanel.title"), 100, 100, 1000, 700);
    ...
    let add_left_btn = create_button(window, &tr("dockpanel.add_left"), 10, y, 120, btn_height);
    log_event("Button", add_left_btn, "Created (Add Left Pane)");

    let add_right_btn = create_button(window, &tr("dockpanel.add_right"), 140, y, 120, btn_height);
    log_event("Button", add_right_btn, "Created (Add Right Pane)");

    let add_top_btn = create_button(window, &tr("dockpanel.add_top"), 270, y, 120, btn_height);
    log_event("Button", add_top_btn, "Created (Add Top Pane)");

    let add_bottom_btn =
        create_button(window, &tr("dockpanel.add_bottom"), 400, y, 120, btn_height);
    log_event("Button", add_bottom_btn, "Created (Add Bottom Pane)");

    let add_center_btn =
        create_button(window, &tr("dockpanel.add_center"), 530, y, 120, btn_height);
    log_event("Button", add_center_btn, "Created (Add Center Pane)");

    let clear_btn = create_button(window, &tr("dockpanel.clear_all"), 660, y, 120, btn_height);
    log_event("Button", clear_btn, "Created (Clear All)");

    y += 35;

    let dock_panel = create_panel(window, 10, y, 980, 480);
    log_event("Panel", dock_panel, "Created (Dock Panel Area)");
    ...
    log_event("Window", window, "Shown - DockPanel Demo Started");
