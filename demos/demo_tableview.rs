//! TableView Demo - Table view widget demonstration with event logging.
...
fn main() {
    ...
    match runtime_gui_mode() {
        RuntimeGuiMode::NativeInteractive => {
            eprintln!("[demo_tableview] running in native-interactive mode");
        }
        RuntimeGuiMode::PreviewOrStub => {
            eprintln!("[demo_tableview] preview/stub mode");
        }
    }

    let window = create_window(&tr("tableview.title"), 100, 100, 900, 600);
    ...
    let add_btn = create_button(window, &tr("tableview.add_row"), 10, y, 120, btn_height);
    log_event("Button", add_btn, "Created (Add Row)");

    let remove_btn = create_button(window, &tr("tableview.remove_row"), 140, y, 120, btn_height);
    log_event("Button", remove_btn, "Created (Remove Row)");

    y += btn_height as i32 + 10;

    let table_label = create_label(window, &tr("tableview.title"), 10, y, 880, btn_height);
    log_event("Label", table_label, "Created");

    y += btn_height as i32 + 5;

    let table_panel = create_panel(window, 10, y, 580, 400);
    log_event("Panel", table_panel, "Created (Table Area)");
    ...
    let log_panel = create_panel(window, 600, y, 290, 400);
    log_event("Panel", log_panel, "Created (Log Area)");

    let log_text = create_line_edit(log_panel, "", 5, 5, 280, 370);
    ...
    log_event("Window", window, "Shown - TableView Demo Started");
