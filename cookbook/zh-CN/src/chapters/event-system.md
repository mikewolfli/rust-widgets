...
`rust-widgets` 事件系统提供了一套全面的分层通信管道：来自平台泵的输入事件、基于 `mpsc` 的解耦投递队列、用于调度的后台事件循环、焦点/指针/定时器管理、文本输入/IME 提交，以及触控到鼠标的事件转换。
...
容器负责决定是否把事件继续转发给子控件；handler 本身不返回 consumed 标志。
