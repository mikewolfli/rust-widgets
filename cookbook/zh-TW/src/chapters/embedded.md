...
rust-widgets 透過 `no_std` 相容設定檔（`mini`）支援嵌入式及資源受限目標平臺，該設定檔包含 `AtomicBool` 全域旗標、固定 DPI 模式、輕量級控制項工廠、硬體輸入處理以及自適應資源管理。
