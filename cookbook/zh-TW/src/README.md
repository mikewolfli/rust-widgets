...
2. **Widget 可用性屬於*設定檔*問題，而非作業系統問題。** 在 `desktop`/`tablet`/`mobile` 設定檔中，全部 180 種 widget 在每個平台上都可使用。只有資源受限的 `embedded`/`mini` 設定檔會編譯精簡的集合。
...
> 下方程式碼片段僅用於說明預期的 API 形式。若需要能對 2.5.3 編譯的程式碼，請從 [`chapters/getting-started.md`](chapters/getting-started.md) 開始，
> 該檔案已針對當前的 crate 驗證過。
...
內建 180 種控件，涵蓋各種常見的 UI 需求，而且**全部都是自繪的，因此每個平台都能使用**：
...
因為所有控件皆為自繪，下列每個平台都支援全部 180 種控件；只有
