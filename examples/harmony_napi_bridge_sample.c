 ...
 *   3) In native callbacks, call rw_harmony_on_node_* APIs.
 */
...
void app_init_ui(void) {
    ...
    g_window = rw_create_window("Harmony Bridge Sample", 80, 80, 720, 480);
    g_button = rw_create_button(g_window, "Tap", 20, 40, 120, 36);
    g_input = rw_create_line_edit(g_window, "", 20, 90, 240, 36);

    /* Build the menu hierarchy with a Quit action. */
    uint64_t menu_bar = rw_create_menu_bar(g_window, 0, 0, 720, 28);
    rw_attach_menu_bar_to_window(g_window, menu_bar);
    uint64_t file_menu = rw_create_menu(menu_bar, "File", 0, 0, 0, 0);
    g_menu_item_quit = rw_menu_add_item(file_menu, "Quit", "cmd+q");

    /* Show the window after building the UI tree. */
    rw_show_widget(g_window);
}
