...
use rust_widgets::core::{Rect, Size, Color, Point};
use rust_widgets::chart::{
    ChartSeries, DataPoint, LineChart, BarChart, PieChart,
...
fn benchmark_chart_series_creation() {
    ...
    assert!(avg_time < 50.0, "Chart series creation should be under 50µs");
}
...
fn benchmark_chart_series_large() {
    ...
    assert!(avg_time < 500.0, "Large chart series creation should be under 500µs");
}
...
fn benchmark_vector_operations() {
    ...
    let avg_time = average_time(ITERATIONS, || {
        ...
        let _variance: f64 = data.iter().map(|x| (x - avg).powi(2)).sum::<f64>() / data.len() as f64;
    });
...
fn benchmark_memory_chart_context() {
    let avg_time = average_time(ITERATIONS, || {
        let mut context = MemoryChartContext::default();
        for i in 0..100 {
            context.draw_line(i as f32, 0.0, i as f32, 100.0, 1.0, Color::rgba(0, 0, 0, 255));
        }
        std::hint::black_box(context);
    });
...
fn benchmark_embedded_config() {
    ...
    assert!(avg_time < 50.0, "EmbeddedConfig creation should be under 50µs");
}
...
fn benchmark_lightweight_config() {
    ...
    assert!(avg_time < 20.0, "LightweightConfig creation should be under 20µs");
}
